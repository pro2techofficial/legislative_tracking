...
$id=(int)($_GET['id'] ?? 0);
$item=mysqli_fetch_assoc(mysqli_query($conn,"SELECT * FROM resolutions WHERE id=$id"));
...
if(isset($_POST['update'])){
    ...
    mysqli_query($conn,"UPDATE resolutions SET resolution_no='$no', title='$title', author='$author', committee='$committee', date_filed='$date_filed', status='$status', file_path='$file', remarks='$remarks' WHERE id=$id");
    ...
    log_action($conn,"Updated resolution $no","Resolution");
    header("Location: index.php"); exit();
}
?>
<div class="topbar"><div><h1>Edit Resolution</h1><p class="subtitle">Update details and legislative status.</p></div><a href="index.php" class="btn gray">Back</a></div>
<div class="panel">
<form method="POST" enctype="multipart/form-data" class="form-grid">
  <div><label>Resolution Number</label><input name="no" value="<?= e($item['resolution_no']) ?>" required></div>
  ...
  <div class="form-full"><button class="btn" name="update">Update Resolution</button></div>
</form>
</div>
