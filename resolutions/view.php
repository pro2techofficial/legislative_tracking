...
$hist=mysqli_query($conn,"SELECT t.*, u.fullname FROM tracking_history t LEFT JOIN users u ON u.id=t.updated_by WHERE legislation_type='Resolution' AND legislation_id=$id ORDER BY t.id DESC");
?>
<div class="topbar"><div><h1>Resolution Details</h1><p class="subtitle">Legislative information and tracking history.</p></div><button onclick="window.print()" class="btn">Print</button></div>
<div class="print-header"><h2>Municipal Legislative Tracking System</h2><p>Resolution Record</p></div>
<div class="panel">
  <h2><?= e($item['resolution_no']) ?> - <?= e($item['title']) ?></h2><br>
