...
if ($search) {
    $items = mysqli_query($conn, "SELECT * FROM resolutions WHERE resolution_no LIKE '%$search%' OR title LIKE '%$search%' OR author LIKE '%$search%' OR committee LIKE '%$search%' OR status LIKE '%$search%' ORDER BY id DESC");
} else {
    $items = mysqli_query($conn, "SELECT * FROM resolutions ORDER BY id DESC");
}
?>
<div class="topbar"><div><h1>Resolutions</h1><p class="subtitle">Manage municipal resolutions and monitor their legislative status.</p></div><a class="btn" href="add.php">Add Resolution</a></div>
...
<tr>
  <td><?= e($r['resolution_no']) ?></td><td><?= e($r['title']) ?></td><td><?= e($r['author']) ?></td><td><?= e($r['committee']) ?></td><td><?= e($r['date_filed']) ?></td>
